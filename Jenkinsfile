  ...
  stages {
   ...
   stage('Get Blue-Green Deployment Repo')
   {
      steps {
        ...
        git(
        url:'git@github.com:jiejayz/blue_green_deployment.git',
        ...
        steps
        {
           ...
           dir('./production')
           {
              script {
                 ...
                 aws_list_bucket = "aws s3api list-buckets --query \"Buckets[].Name\" | grep \"01-prod-uction\" | awk '{print \$1}'  | awk '{print substr(\$1,2); }' | awk '{print substr(\$1, 1, length(\$1)-2)}'"
                 ...
                 if ("$bucketResult".toString().equals("01-prod-uction")) {  
                    echo "terraform bucket already exists."
                 } else {
                    sh 'aws s3 mb s3://01-prod-uction --region us-west-1'
...
//	            sh 'aws s3 mb s3://01-prod-uction-blue --region us-west-1'
