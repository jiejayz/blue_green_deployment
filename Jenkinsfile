        ...
        steps
        {
           ...
           dir('./production')
           {
              script {
                 ...
                 aws_list_bucket = "aws s3api list-buckets --query \"Buckets[].Name\" | grep \"01-prod\" | awk '{print \$1}'  | awk '{print substr(\$1,2); }' | awk '{print substr(\$1, 1, length(\$1)-2)}'"
                 ...
                 if ("$bucketResult".toString().equals("01-prod")) {  
                    echo "terraform bucket already exists."
                 } else {
                    sh 'aws s3 mb s3://01-prod --region us-west-1'
